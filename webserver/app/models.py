...
class FTDay(db.Model):
    __tablename__ = 'featday'
    day = db.Column(db.String(100), primary_key=True, nullable=False, index=True)
    ft  = db.Column(db.String(5000))
    ...
    def generate_fake(filename=''):
        ...
        for line in open(filename):
            ...
            record = FTDay.query.filter_by(day=arr[0]).first()
            if record is None:
                db.session.add(f)
            else:
                record.ft = arr[1]
...
class SUMMARY(db.Model):
    ...
    def __repr__(self):
        return '<SUMMARY {}>'.format(self.tp)

class PREDICT(db.Model):
    __tablename__ = 'preday'
    day   = db.Column(db.String(100), primary_key=True, nullable=False, index=True)
    true  = db.Column(db.Integer())
    pred  = db.Column(db.Integer())
    prop  = db.Column(db.Float())
    nprd  = db.Column(db.Integer())
    pday  = db.Column(db.String(200))
    uday  = db.Column(db.String(100))

    @staticmethod
    def generate_fake(filename='', uday=''):
        from sqlalchemy.exc import IntegrityError
        lastday = PREDICT.query.order_by(PREDICT.day.desc()).first()
        num = 0
        add = 0
        for line in open(filename):
            arr = line.strip().split(',')
            ...
            record = PREDICT.query.filter_by(day=arr[0]).first()
            if record is None:
                db.session.add(f)
                add = 1
            else:
                if arr[0] == lastday.day and add == 0:
                    record.uday = uday
                record.true = arr[1]
                record.pred = arr[2]
                record.prop = arr[3]
    ...
    def __repr__(self):
        return '<PREDICT {}>'.format(self.day)
...
class STOCKS(db.Model):
    ...
    def generate_fake(filename=''):
        ...
        for line in open(filename):
            ...
            record = DAPAN.query.filter_by(day=arr[0]).first()
            if record is None:
                db.session.add(f)
            else:
                record.flag = arr[1]
    ...
    def __repr__(self):
        return '<DAPAN {}>'.format(self.day)


class DAPAN(db.Model):
    __tablename__ = 'dapanday'
    day   = db.Column(db.String(100), primary_key=True, nullable=False, index=True)
    flag  = db.Column(db.Integer())
    prop  = db.Column(db.Float())
...
class FTdict(db.Model):
    __tablename__ = 'featdict'
    fid    = db.Column(db.String(100), primary_key=True, nullable=False, index=True)
    fname  = db.Column(db.String(100))
    fdesc  = db.Column(db.String(900))
    fscore = db.Column(db.Float())
    ...
    def __repr__(self):
        return '<FTdict {}>'.format(self.fid)

class Tbuy(db.Model):
    __tablename__ = 'tb_stock_day_kline_shape'
    __table_args__ = (db.UniqueConstraint('stock_id', 'day'), {})
    id = db.Column(db.Integer, primary_key=True)
    stock_id = db.Column(db.Integer, nullable=False, index=True)
    day = db.Column(db.Integer)
    shape_type = db.Column(db.String(100))
    shape_json = db.Column(db.String(1024))
    ...
    def __repr__(self):
        return '<Tbuy {}>'.format(self.stock_id)
